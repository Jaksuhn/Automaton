...
public static class CustomNodes
{
    ...
    private static readonly Dictionary<string, uint> NodeIds = [];
    private static readonly Dictionary<uint, string> NodeNames = [];
    private static uint _nextId = 0x53541000;

    public static uint Get(BaseFeature tweak, string label = "", int index = 0) => string.IsNullOrEmpty(label) ? Get($"{tweak.GetType().Name}", index) : Get($"{tweak.GetType().Name}::{label}", index);
    ...
    public const int
        ...
        FelicitousToken = SimpleTweaksNodeBase + 420,
